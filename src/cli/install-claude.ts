...
/**
 * Writes or updates the Claude Desktop MCP server config to include caldav-mcp.
 *
 * @param configPath - Absolute path to claude_desktop_config.json
 * @param binaryPath - Absolute path to the caldav-mcp binary
...
export async function installClaude(configPath: string, binaryPath: string): Promise<string> {
  ...
  (config.mcpServers as Record<string, unknown>).caldav = { command: binaryPath };
